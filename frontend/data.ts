import { Product } from './src/types/Product'

export const earphones: Product[] = [
  {
    name: 'YX1 Wireless Earphones',
    slug: 'yx1-wireless-headphones',
    description: 'Wireless headphones',
    price: 599,
    image: './public/images/shared/desktop/image-xx59-headphones.jpg',
    rating: 0,
    numReviews: 0,
    category: 'Speakers',
  },
]

export const headPhones: Product[] = [
  {
    name: 'XX99 Mark II Headphones',
    slug: 'mark-2-headphones',
    description: 'Noise cancelling headphones',
    price: 2999,
    image: './public/images/shared/desktop/image-xx99-mark-two-headphones.jpg',
    rating: 0,
    numReviews: 0,
    category: 'Headphones',
  },
  {
    name: 'XX59 Headphones',
    slug: 'xx59-headphones',
    description: 'Noise cancelling headphones',
    price: 899,
    image: './public/images/shared/desktop/image-xx59-headphones.jpg',
    rating: 0,
    numReviews: 0,
    category: 'Headphones',
  },
  {
    name: 'XX99 Mark I Headphones',
    slug: 'mark-1-headphones',
    description: 'Noise cancelling headphones',
    price: 1750,
    image: './public/images/shared/desktop/image-xx99-mark-one-headphones.jpg',
    rating: 0,
    numReviews: 0,
    category: 'Headphones',
  },
]

export const speakers: Product[] = [
  {
    name: 'ZX9 Speaker',
    slug: 'zx9-speaker',
    description: 'medium sized Speaker',
    price: 4500,
    image: './public/images/shared/desktop/image-zx9-speaker.jpg',
    rating: 0,
    numReviews: 0,
    category: 'Speakers',
  },
  {
    name: 'ZX7 Speaker',
    slug: 'zx7-speaker',
    description: 'small sized Speaker',
    price: 3500,
    image: './public/images/shared/desktop/image-zx7-speaker.jpg',
    rating: 0,
    numReviews: 0,
    category: 'Speakers',
  },
]
